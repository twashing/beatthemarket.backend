...
(defn ->user [email name external-uid games]
...
(defn collect-realized-profit-loss-for-user-allgames
  ...
  ([conn email]
   ...
   (collect-realized-profit-loss-for-user-allgames conn email false))

  ([conn email group-by-stock?]

   (let [user-game-tuples->user (comp :game.user/user first :game/users first)
         user-games->user-with-games
         (fn [games]

           (let [{user-db-id :db/id
                  email :user/email
                  name :user/name
...
(defn realized-profit-loss-for-game [conn user-id game-id]
  ...
  (->> (d/q '[:find (pull ?pls [:db/id
                                ...
                                {:game.user.profit-loss/tick
                                 [:game.stock.tick/id]}
                                {:game.user.profit-loss/stock
                                 [:game.stock/id
                                  :game.stock/name
                                  :game.stock/data-seed
                                  :game.stock/symbol
                                  {:game.user.profit-loss/type [*]}]}])
              :in $ ?gameId ?user-id
              :where
              [?g :game/id ?gameId]
              [?g :game/users ?gus]
