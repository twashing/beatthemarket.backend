...
(deftest win-game-test
  ...
  (let [;; A
        ...
        ;; B
        data-sequence-fn (constantly [100.0 1500100.0])
        tick-length      (count (data-sequence-fn))
...
#_(deftest buy-stock!-multiple-test
    ...
    (let [;; A
          ...
          [_ iterations] (game.games/start-workbench! conn user-db-id game-control)
          {stock-idA   :game.stock/id
           stockNameA :game.stock/name} (first stocks)
          {stock-idB   :game.stock/id
      ...
      (testing "Returned Tentry matches what was submitted"
        (let [{tickPriceA :game.stock.tick/close
              ...
              pl-entries ((juxt
                            #(get % stock-idA)
                            #(get % stock-idB)) game-profit-loss)]
